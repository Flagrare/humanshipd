...
Matching is **tiered**, not all-or-nothing. An exact byte match is the strongest result, but the content fingerprint now backs it up: verification reports *exact file*, *same content* (reformatted or lightly edited), *same writing* (heavily edited), *borderline — needs review*, or *no match*, with the measured distance and a published threshold. So the same writing re-saved with different whitespace, or revised a little, still verifies — as a content match rather than a flat "invalid". The remaining gap is reading text out of binary formats: a `.docx` or `.pdf` of the same writing needs its text extracted before the fingerprint can compare, and that per-format extraction is the next step ([content-binding design](docs/superpowers/specs/2026-06-06-content-binding-and-capture-fidelity-design.md)).
