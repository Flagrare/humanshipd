...
Matching is **tiered**, not all-or-nothing. An exact byte match is the strongest result, but the content fingerprint now backs it up: verification reports *exact file*, *same content* (reformatted or lightly edited), *same writing* (heavily edited), *borderline — needs review*, or *no match*, with the measured distance and a published threshold. So the same writing re-saved with different whitespace, or revised a little, still verifies — as a content match rather than a flat "invalid". Verification also reads the text out of common containers before comparing: `.txt` and `.docx` work everywhere (the browser validator unzips Word files in-page), and `.pdf` works with the command-line tool. So a `.docx` export of the same writing now verifies as a content match. Extracting headers/footers and tracked changes precisely, and PDF support in the browser, are the remaining refinements ([content-binding design](docs/superpowers/specs/2026-06-06-content-binding-and-capture-fidelity-design.md)).
...
The **core** is written in Rust and contains the entire credential format, signing, and verification, so a credential made by one capture method is byte-for-byte identical to one made by another. That same core compiles to WebAssembly, so the **browser extension signs credentials in-page** (in a Web Worker) and the verify page checks them — both running the exact Rust logic, with no native helper to install. Because the core (and the registry) is plain Rust with no OS-specific code, it builds and runs on macOS, Windows, and Linux alike; only the capture adapters are platform-specific. Those adapters are deliberately thin: a **macOS Accessibility adapter** for native apps like Word and Scrivener, and a **browser extension** for ordinary web editors *and Google Docs* (which renders to `<canvas>`, so the extension reads its save stream instead), are both built; **Windows** (UI Automation) and **Linux** (AT-SPI) native adapters are planned. Where an app exposes nothing else, screenshots with on-device text recognition are the last-resort fallback.
...
**Working today:** the Rust core — building the process summary, issuing a signed C2PA credential bound to a file, verifying it, embedding a credential invisibly in text, and computing the ISCC durable fingerprint. An opt-in registry service for fingerprint → credential lookup, with end-to-end recovery proven. A **macOS capture tool** that runs the whole slice for real: it reads your live typing in TextEdit or Word, issues a signed credential bound to the document, and verifies it. A **browser extension** that captures the same way in ordinary web editors (typed vs. pasted, with caret position) **and Google Docs** (reading its live save stream, since Docs hides text in a `<canvas>`) — where it now **remembers your writing across sessions**, so closing the tab and reopening the same doc keeps accumulating onto one record instead of starting over — then **signs the credential in your browser** — in a WASM Web Worker, with no native host to install — optionally attaching a self-asserted author name, and downloads it as a single `.zip`. If a Doc already had content before the extension started watching, it honestly **declines to issue** rather than vouch for writing it never saw.
