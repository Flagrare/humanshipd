...
- **Relevance:** high (load-bearing limitation)
- **What this contributed:** Google Docs renders text to canvas — the visible text is **not in the DOM or the standard accessibility tree**; a hidden a11y DOM appears only when the user enables screen-reader mode. So AX extraction of Google Docs is unreliable by default, forcing **OCR fallback** for one of the most popular writing surfaces.

## Synthesis

**An app-agnostic, on-device, open-source capture layer is feasible and has shipped (screenpipe, Rewind, Glass). The capture is the easy ~80%; the attestation semantics on top are the hard, valuable, partly-unsolved 20%.** The most feasible architecture is a **combination**, not any single modality:

1. **Primary — Accessibility API** (macOS AXUIElement, Windows UIA, Linux AT-SPI/GNOME): read the focused text element's content + change events from an **allow-listed, in-focus writing app**. Cleaner, cheaper, and more privacy-respecting than screenshots (reads one field, not the whole screen). This is screenpipe's accessibility-first model.
2. **Correlate — Input monitoring (timing only, never content)**: distinguish text that appeared *with* keystrokes (typed) from text that appeared *without* (pasted) — the AI-dump detector. macOS CGEventTap (Input Monitoring); equivalents elsewhere.
3. **Fallback — Screenshot + on-device OCR** (Vision / Windows.Media.Ocr / Tesseract): only for canvas/poor-AX apps, notably **Google Docs**, remote desktops, etc.

**Per-OS reality:** Windows is the least intrusive (UIA needs no scary prompt; OCR needs MSIX). macOS is the highest friction (Accessibility + Input Monitoring grants, sandbox off → **no Mac App Store**, Developer ID + notarization, periodic Sequoia re-prompts). Linux works under GNOME/Wayland with restore tokens but is the least reliable across DEs.
...
- **Google Docs** defeats AX → OCR fallback required for a major writing surface.
- **Granularity:** screenshot diffing only resolves bursts; per-keystroke timing needs input monitoring (extra permission).
- **Permission irony:** an anti-AI-surveillance tool needs spyware-grade permissions. Mitigation for a privacy-first OSS project: auditable code, AX-over-screenshots, allow-list only the focused writing app, store only hashes/timestamps, zero network except the stateless tamper-check.
